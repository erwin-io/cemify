...
export class MapBoxComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    forkJoin([
      ...
      // console.log(res);
      const a = res[0].data.sort((a, b) => Number(a.lotId) - Number(b.lotId)).map(res=> this.createBox(res));
      const b = res[1].data.sort((a, b) => Number(a.lotId) - Number(b.lotId)).map(res=> this.createBox(res));
      const c = res[2].data.sort((a, b) => Number(a.lotId) - Number(b.lotId)).map(res=> this.createBox(res));
      const d = res[3].data.sort((a, b) => Number(a.lotId) - Number(b.lotId)).map(res=> this.createBox(res));
  ...
  ngAfterViewInit() {
    this.panZoom = Panzoom(this.map.nativeElement, { contain: 'outside', startScale: 1.5, maxScale: 20 });
